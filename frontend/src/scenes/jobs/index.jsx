...
const Jobs = () => {
    ...
    const columns = [
        ...
        { field: "status", renderHeader: () => <Typography sx={{ fontWeight: 'bold' }}>{'Status'}</Typography>, flex: .6, },
        ...
        { field: "run_time", renderHeader: () => <Typography sx={{ fontWeight: 'bold' }}>{'Run Time'}</Typography>, flex: .5, },
        {
            ...
            renderCell: (params) => (
                <Stack direction="row" spacing={1} alignItems={'center'} height={'100%'}>
                    <DeleteIcon
                        onClick={() => handleDeleteJob(params.row.job_name, params.row.scheduled_start_time)}
                        sx={{ cursor: 'pointer', color: colors.redAccent[500], alignItems: 'center' }}
                    />
                </Stack>
            ),
            flex: .5,
    ...
    const handleDeleteJob = async (job_name, scheduled_start_time) => {
        if (window.confirm(`Are you sure you want to delete the job: ${job_name}?`)) {
            try {
